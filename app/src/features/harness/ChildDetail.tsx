...
export function ChildDetail({ sessionId, child, details, observedAtMs, tab, pendingKey, onBack, onTab, onAction }: {
  ...
  return <div className="child-detail">
    <header className="child-detail-nav"><button type="button" data-control-id={back.controlId} aria-label="Back to Harness" onClick={onBack}>←</button><button type="button" data-control-id={`${back.controlId}:crumb`} onClick={onBack}>Harness</button><span>/</span><strong>{child.task}</strong><button type="button" data-control-id={`${back.controlId}:close`} aria-label="Close child detail" onClick={onBack}>×</button></header>
    <div className="child-detail-scroll">
      ...
      {details?.error && <section className="child-error" role="alert"><strong>{details.error.code}</strong><p>{details.error.message}</p>{details.error.code === "server_is_overloaded" ? <button type="button" onClick={() => onAction({ action: "harness.overload.retry", payload: { sessionId, errorId: details.error!.code } }, `child-retry:${child.id}`)}>Retry task</button> : details.error.retryable ? <button type="button" disabled title="The runtime did not provide a retry admission handle.">Retry task</button> : null}</section>}
      ...
      <section className="child-panel" role="tabpanel" aria-label={`${tab} for ${child.task}`}>
        ...
        {tab === "files" && (details?.files.length ? <div className="child-files">{details.files.map((file) => { const open = createControlBinding(`editor.artifact.open:${file.id}`, "editor.artifact.open"); return <button type="button" data-control-id={open.controlId} key={file.id} aria-label={`Open ${file.path}`} onClick={() => onAction({ action: "editor.artifact.open", payload: { sessionId, artifactId: file.id } }, `file:${file.id}`)}><span>{file.path}</span><small>{file.change}</small></button>; })}</div> : <p>No files touched yet.</p>)}
      </section>
    </div>
    <footer className="child-detail-footer"><div><span aria-hidden="true">▣</span><span>Child tasks are managed by the harness</span></div>{(child.status === "running" || child.status === "queued") && <button type="button" data-control-id={stop.controlId} className="child-stop" disabled={pendingKey === `child-stop:${child.id}`} onClick={() => onAction({ action: "harness.child.stop", payload: { sessionId, childId: child.id } }, `child-stop:${child.id}`)}>{pendingKey === `child-stop:${child.id}` ? "Stopping…" : "Stop task"}</button>}</footer>
  </div>;
}
