...
export function GeneralSettings({ settings, onSetting }: { readonly settings: AppSettings; readonly onSetting?: SettingWriter }) {
  return <><SettingGroup title="General"><Row label="Default file open destination" description="Choose where verified files and folders open."><SettingSelect label="Default file open destination" value={settings.fileOpenDestination ?? "system"} setting="fileOpenDestination" onSetting={onSetting} options={[{ value: "system", label: "System default" }, { value: "vscode", label: "VS Code" }, { value: "studio", label: "Prime Studio" }]} /></Row><Row label="Language" description="Language used by the application interface."><SettingSelect label="Language" value={settings.language ?? "auto"} setting="language" onSetting={onSetting} options={[{ value: "auto", label: "Auto detect" }, { value: "en", label: "English" }]} /></Row><Row label="Bottom panel" description="Show the terminal and diagnostics panel control in the app header."><SettingSwitch label="Bottom panel" enabled={boolValue(settings.bottomPanel, false)} setting="bottomPanel" onSetting={onSetting} /></Row></SettingGroup><SettingGroup title="Workspace"><Row label="Default workspace" description="New chats start in this user-selected working directory."><span className="studio-setting-value">{settings.defaultCwd || "Ask each time"}</span></Row><Row label="Panel layout" description="Pane widths and open states are persisted per window and reflow at narrow widths."><span className="studio-setting-value">Adaptive</span></Row></SettingGroup></>;
}
...
export function AccountsSettings({ accounts, defaultAccount, onChanged, onDefaultAccount }: { readonly accounts: readonly Account[]; readonly defaultAccount: string | null; readonly onChanged: (accounts?: Account[]) => void; readonly onDefaultAccount: (accountId: string | null) => void }) {
  return <div className="studio-accounts-settings"><Accounts accounts={[...accounts]} onChanged={onChanged} onUse={() => undefined} newSessionDisabledReason="New session activation is not connected yet." defaultAccount={defaultAccount} onDefaultAccount={onDefaultAccount} /></div>;
}
...
export function HarnessSettings({ compatibility, settings, onSetting }: { readonly compatibility: HarnessCompatibility; readonly settings: AppSettings; readonly onSetting?: SettingWriter }) {
  const canConfigure = connected(compatibility);
  const reason = "A verified Harness connection is required.";
  return <>{!canConfigure && <Unavailable>{reason} Existing values remain visible but cannot be changed.</Unavailable>}<SettingGroup title="Agent runtime"><Row label="Maximum concurrent subagents" description="Studio requests this ceiling; the Harness remains the final admission authority."><input aria-label="Maximum concurrent agents" type="number" min={1} max={64} value={settings.maxConcurrentAgents ?? "4"} disabled={!canConfigure || !onSetting} onChange={(event) => onSetting?.("maxConcurrentAgents", event.currentTarget.value)} /></Row><Row label="Autonomous maximum turns" description="Limit unattended turns before control returns to you."><input aria-label="Autonomous maximum turns" type="number" min={1} max={1000} value={settings.autonomousMaxTurns ?? "40"} disabled={!canConfigure || !onSetting} onChange={(event) => onSetting?.("autonomousMaxTurns", event.currentTarget.value)} /></Row><Row label="Retry silent worker death" description="Retry once when the verified runtime reports an unexpected clean worker exit."><SettingSwitch label="Retry silent worker death" enabled={boolValue(settings.retrySilentWorkers)} setting="retrySilentWorkers" onSetting={onSetting} disabled={!canConfigure} reason={reason} /></Row><Row label="Context discovery" description="Honor repository AGENTS.md instructions before broader workspace context."><SettingSelect label="Context discovery" value={settings.contextDiscovery ?? "agents-first"} setting="contextDiscovery" onSetting={onSetting} disabled={!canConfigure} reason={reason} options={[{ value: "agents-first", label: "AGENTS.md wins" }, { value: "project-only", label: "Project only" }]} /></Row></SettingGroup></>;
}

export function ModelsSettings({ compatibility, settings, onSetting }: { readonly compatibility: HarnessCompatibility; readonly settings: AppSettings; readonly onSetting?: SettingWriter }) {
  const canChoose = connected(compatibility) && compatibility.capabilities.includes("model_catalog");
  return <>{!canChoose && <Unavailable>A verified model catalog is required. Prime Studio will not invent provider or context-window choices.</Unavailable>}<SettingGroup title="Defaults"><Row label="Provider" description="Provider used for newly created sessions."><SettingSelect label="Default provider" value={settings.defaultProvider ?? "unavailable"} setting="defaultProvider" onSetting={onSetting} disabled={!canChoose} options={[{ value: "unavailable", label: "Unavailable" }, { value: "openai-codex", label: "OpenAI Codex" }, { value: "anthropic", label: "Anthropic" }]} /></Row><Row label="Model" description="Model catalog and context limits come from the verified Harness."><span className="studio-setting-value">{settings.defaultModel || "Not reported"}</span></Row><Row label="Thinking level" description="Effort values are validated against the selected model."><SettingSelect label="Default thinking level" value={settings.defaultThinking ?? "medium"} setting="defaultThinking" onSetting={onSetting} disabled={!canChoose} options={["off", "low", "medium", "high", "max"].map((value) => ({ value, label: value[0]!.toUpperCase() + value.slice(1) }))} /></Row></SettingGroup></>;
}
...
export function ToolsSettings({ compatibility, settings, onSetting }: { readonly compatibility: HarnessCompatibility; readonly settings: AppSettings; readonly onSetting?: SettingWriter }) {
  const canConfigure = connected(compatibility);
  return <><Unavailable>{canConfigure ? "Tool availability is projected by the active session; policy-locked tools remain read-only." : "Connect a verified Harness to inspect and configure its tools."}</Unavailable><SettingGroup title="Harness tools"><Row label="Enable configurable tools" description="Allow tools admitted by both workspace policy and the verified runtime."><SettingSwitch label="Enable configurable tools" enabled={boolValue(settings.toolsEnabled)} setting="toolsEnabled" onSetting={onSetting} disabled={!canConfigure} /></Row><Row label="Computer use and browser" description="High-impact tools remain subject to their native authority and cannot be enabled by this toggle alone."><span className="studio-setting-value">Policy controlled</span></Row></SettingGroup></>;
}

export function GitSettings({ settings, onSetting }: { readonly settings: AppSettings; readonly onSetting?: SettingWriter }) {
  return <SettingGroup title="Repository"><Row label="Automatic status refresh" description="Refresh read-only Git status when the active workspace changes."><SettingSwitch label="Automatic Git status refresh" enabled={boolValue(settings.gitAutoRefresh)} setting="gitAutoRefresh" onSetting={onSetting} /></Row><Row label="Write operations" description="Commit, push, reset, and checkout require an explicit command and native authority."><span className="studio-setting-value">Never automatic</span></Row></SettingGroup>;
}

export function EnvironmentsSettings({ settings, onSetting }: { readonly settings: AppSettings; readonly onSetting?: SettingWriter }) {
  return <SettingGroup title="Agent environment"><Row label="Environment" description="Choose where verified Harness processes run on Windows."><SettingSelect label="Agent environment" value={settings.environmentMode ?? "windows-native"} setting="environmentMode" onSetting={onSetting} options={[{ value: "windows-native", label: "Windows native" }, { value: "wsl", label: "WSL (when available)" }]} /></Row><Row label="Default working directory" description="The current default path is stored without credentials."><span className="studio-setting-value studio-setting-path">{settings.defaultCwd || "Ask each time"}</span></Row></SettingGroup>;
}
