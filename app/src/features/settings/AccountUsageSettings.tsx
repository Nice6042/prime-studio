...
function UnavailableFact({ name, children }: { readonly name: string; readonly children: string }) {
  return <div className="studio-usage-unavailable-fact" role="note" aria-label={`${name} unavailable`} tabIndex={0}>
...
export function AccountUsageSettings({ accounts, onExportCsv, loadUsage = rpc.accountUsageSeriesStrict }: {
  ...
  return <>
    ...
    <section className="studio-usage-chart"><header><div><h2>Daily {metric}</h2><span>Peak {available ? (metric === "cost" ? money.format(peak) : compact.format(peak)) : loading ? "refreshing" : "unavailable"}</span></div><div className="studio-chart-legend" role="group" aria-label="Chart series"><button type="button" data-control-id={controls.series.controlId} data-action={controls.series.action} aria-pressed="true" onClick={() => setNotice(null)}><i />Account history</button><button type="button" data-control-id="settings.usage.series.subagents" data-action={controls.series.action} aria-disabled="true" aria-label="Subagents unavailable" onClick={() => unavailableSeries("Subagent")}>Subagents</button><button type="button" data-control-id="settings.usage.series.tools" data-action={controls.series.action} aria-disabled="true" aria-label="Tools unavailable" onClick={() => unavailableSeries("Tool")}>Tools</button></div></header>{available ? <><svg data-chart="account-usage" role="img" aria-label={`Daily ${metric} over ${days} days`} viewBox="0 0 720 180" preserveAspectRatio="none"><title>{`Daily ${metric} over ${days} days`}</title>{[0, 1, 2, 3].map((line) => <path key={line} className="studio-chart-grid" d={`M0 ${line * 56 + 6} H720`} />)}<path className="studio-chart-area" d={chartPath ? `${chartPath} L720,180 L0,180 Z` : ""} /><path className="studio-chart-line" d={chartPath} /></svg><footer><span>{day.format(daily[0]?.timestamp ?? Date.now())}</span><span>{day.format(daily[daily.length - 1]?.timestamp ?? Date.now())}</span></footer></> : <p className="studio-usage-chart-empty">{loading ? "Refreshing the verified account ledger…" : "Daily series is unavailable until a verified account ledger is readable."}</p>}</section>
    <section className="studio-usage-breakdown"><header><h2>Breakdown by provider</h2><button type="button" data-control-id={controls.export.controlId} data-action={controls.export.action} disabled={!onExportCsv || loading} title={!onExportCsv ? "A native user-selected save destination is not connected." : undefined} onClick={() => void exportCsv()}>Export CSV</button></header><table><thead><tr><th>Provider</th><th>{metric === "cost" ? "Cost" : "Tokens"}</th><th>Share</th></tr></thead><tbody>{providerTotals.length === 0 ? <tr><td colSpan={3}>No verified usage in this window.</td></tr> : providerTotals.map(({ provider, totals: providerValue }) => <tr key={provider}><td>{PROVIDER_NAME[provider] ?? provider}</td><td>{display(providerValue)}</td><td>{value(totals) > 0 ? `${(value(providerValue) / value(totals) * 100).toFixed(1)}%` : "—"}</td></tr>)}</tbody></table></section>
    <section className="studio-usage-breakdown"><header><h2>Usage by account</h2></header><table><thead><tr><th>Account</th><th>Provider</th><th>{metric === "cost" ? "Cost" : "Tokens"}</th></tr></thead><tbody>{accountTotals.map(({ account, totals: accountValue }) => <tr key={account.id}><td>{account.label}</td><td>{PROVIDER_NAME[account.provider] ?? account.provider}</td><td>{available ? display(accountValue) : "Unavailable"}</td></tr>)}</tbody></table></section>
    <section className="studio-usage-unsupported" aria-label="Unavailable account-usage dimensions"><UnavailableFact name="Model breakdown">Model identity is not reported by the current account ledger.</UnavailableFact><UnavailableFact name="Project breakdown">Project identity is not reported by the current account ledger.</UnavailableFact><UnavailableFact name="Subscription quota">The account ledger does not report provider quota; cost and quota are separate facts.</UnavailableFact></section>
