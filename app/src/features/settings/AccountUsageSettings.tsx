...
export function AccountUsageSettings({ accounts, onExportCsv, loadUsage = rpc.accountUsageSeriesStrict }: {
  ...
  return <>
    ...
    <section className="studio-usage-chart"><header><div><h2>Daily {metric}</h2><span>Peak {available ? (metric === "cost" ? money.format(peak) : compact.format(peak)) : loading ? "refreshing" : "unavailable"}</span></div><div className="studio-chart-legend" role="group" aria-label="Chart series"><button type="button" data-control-id={controls.series.controlId} data-action={controls.series.action} aria-pressed="true" onClick={() => setNotice(null)}><i />Account history</button><button type="button" data-control-id="settings.usage.series.subagents" data-action={controls.series.action} disabled aria-label="Subagents unavailable" title="The account ledger does not report subagent attribution.">Subagents</button><button type="button" data-control-id="settings.usage.series.tools" data-action={controls.series.action} disabled aria-label="Tools unavailable" title="The account ledger does not report tool attribution.">Tools</button></div></header>{available ? <><svg data-chart="account-usage" role="img" aria-label={`Daily ${metric} over ${days} days`} viewBox="0 0 720 180" preserveAspectRatio="none"><title>{`Daily ${metric} over ${days} days`}</title>{[0, 1, 2, 3].map((line) => <path key={line} className="studio-chart-grid" d={`M0 ${line * 56 + 6} H720`} />)}<path className="studio-chart-area" d={chartPath ? `${chartPath} L720,180 L0,180 Z` : ""} /><path className="studio-chart-line" d={chartPath} /></svg><footer><span>{day.format(daily[0]?.timestamp ?? Date.now())}</span><span>{day.format(daily[daily.length - 1]?.timestamp ?? Date.now())}</span></footer></> : <p className="studio-usage-chart-empty">{loading ? "Refreshing the verified account ledger…" : "Daily series is unavailable until a verified account ledger is readable."}</p>}</section>
