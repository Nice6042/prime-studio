...
function setup(outcome: StudioOperationOutcome = { status: "updated", revision: 1 }, newChatDisabledReason?: string) {
  ...
  const commands: StudioCommandId[] = [];
  const runCommand = vi.fn((id: StudioCommandId) => { commands.push(id); });
  render(<CollapsedSidebar
    newChatDisabledReason={newChatDisabledReason}
    workspace={workspace}
    workspaceMenuOpen={false}
    onCommand={runCommand}
    onExecuteWorkspaceOperation={execute}
  />);
  return { execute, operations, runCommand, commands };
}
...
describe("CollapsedSidebar", () => {
  ...
  it("exposes the exact five-action rail once, with authored accessible tooltips", () => {
    ...
    expect(actions.map((button) => button.getAttribute("data-control-id"))).toEqual([
      "rail.sidebar.toggle", "rail.chat.new", "rail.palette.open", "rail.settings.open", "rail-workspace-menu",
    ]);
  ...
  it("projects every command-owned rail label, hint and action from the registry", () => {
    setup();
    const expected = [
      ["Expand sidebar", "Ctrl+B", "layout.sidebar.toggle", "rail.sidebar.toggle"],
      ["New chat", "Ctrl+N", "catalog.chat.create", "rail.chat.new"],
      ["Search", "Ctrl+K", "palette.open", "rail.palette.open"],
      ["Settings", "Ctrl+,", "route.settings.open", "rail.settings.open"],
    ] as const;
    for (const [label, hint, action, placementId] of expected) {
      ...
      expect(button).toHaveAccessibleDescription(expect.stringContaining(hint));
    }
  });
  ...
  it("keeps unavailable New chat focusable and explains why without dispatching", async () => {
    const { runCommand } = setup({ status: "updated", revision: 1 }, "Choose a default account first.");
