...
# The shim is an optional local patch — skip the --require pair when it is absent.
$shim = Join-Path $dist "windowshide-shim.cjs"
$req = if (Test-Path $shim) { "--require `"$shim`" " } else { "" }
$args = "$req`"$cli`" -p --provider anthropic --model claude-opus-5 --cwd `"$scratch`" `"$Prompt`""
